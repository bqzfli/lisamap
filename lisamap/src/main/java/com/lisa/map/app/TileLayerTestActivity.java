...
public class TileLayerTestActivity extends Activity 
...
MultipleItemChangedListener {
	...
	IFeatureLayer layerZRDKShp = null;
	IFeatureLayer layerYFShp = null;
	IFeatureLayer layerCUNShp = null;
	...
	private String mDirSurveyLayers = MapUtil.SURVEYDATAPATH+"/测试区/";
	private String mDirBasicLayers = MapUtil.BASICDATAPATH+"/测试区/";
	private String mDirWMTSLayers = MapUtil.BASEPATH+"/WMTS/";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try{
			...
			setWMTSLayers();
			loadSurveyLayer(this.mDirSurveyLayers);
			loadBasicLayer(this.mDirBasicLayers);
			loadDBLayer(MapUtil.PATH_RESULT_DB, MapUtil.TABLE_YFZRDKBD_NAME);
	...
	private void loadDBLayer(String dbPath,String tableName) throws Exception{
		mDBLAYER = new DBLayer(MapUtil.TABLE_YFZRDKBD_NAME);
		mDBLAYER.initInfos( 
				MapUtil.PATH_RESULT_DB, 
				MapUtil.TABLE_YFZRDKBD_NAME,
		...
		/*TouchLongToolCommen touchLongTool = new TouchLongToolCommen(COMMONLAYER);
	 ...
	 * @param path 矢量数据文件夹
	 */
	private void loadSurveyLayer(String dirPath){
		//读取数据
		try {
			layerCUNShp = new FeatureLayer(dirPath+"/村边界.shp", null);
			((SimpleRenderer)layerCUNShp.getRenderer()).setSymbol(Setting.SYMBOLCUN);
			//			Setting.SetLayerLabel(layerCUNShp, "CUNMC", 12, Color.argb(255, 200,224,255), null);

			layerYFShp = new FeatureLayer(dirPath +"/样本村样方.shp", null);
			((SimpleRenderer)layerYFShp.getRenderer()).setSymbol(Setting.SYMBOLYF);
			//			Setting.SetLayerLabel(layerYFShp, "YFBH", 12, Color.RED, null);
			/*layerYFShp.setMinimumScale(3000);*/
			layerYFShp.setMaximumScale(30000);

			layerZRDKShp = new FeatureLayer(dirPath +"/样方自然地块.shp", null);
	 ...
	 */
	private void loadBasicLayer(String dirPath){
	...
	protected void onResume() {
		...
		try {
			loadBasicMap(/*this.layerTDT*/layerChinaOnlineCommunity/*null*/,
					(IEnvelope)layerCUNShp.getFeatureClass().getGeometry(
							layerCUNShp.getFeatureClass().getFeatureCount()-1
							).Extent().Buffer(100));
		} catch (Exception e) {
